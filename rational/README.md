...
класса ошибки библиотеки C++. Если не всё ясно сейчас, то это ok, если не всё понятно перед зачетом или экзаменом —
 у вас проблемы.
...
1. Решение должно состоять из файлов `rational.hpp` (объявления и элементарные определения/реализации) и
 `rational.cpp` (определения всего остального).

2. Хорошей идеей будет вынести логику сокращения дроби в отдельный метод (например, *Reduce()*).

3. Для вычисления НОД используйте `std::gcd`.

4. Гарантируется, что числитель и знаменатель в любой момент помещаются в `int32_t`.

5. Стиль кода запрещает неявные конструкторы преобразования (и правильно делает), но в этой задаче неявное преобразование
из целых чисел в рациональное вполне осмысленно. Чтобы проверки игнорировали строки с объявлением такого
конструктора, добавьте комментарий `// NOLINT` после заголовка метода.

6. Подумайте над каждым методом/функцией, можно ли её пометить спецификатором *noexcept*, можно ли результат метода/функции
пометить спецификатором *constexpr*; а если метод/функция возвращает результат, то уместен ли будет атрибут *[[nodiscard]]*?

7. В задаче нет скрытых тестов - все тесты публичные (`rational_test.cpp`).